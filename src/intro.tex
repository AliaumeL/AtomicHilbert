...
For instance, the ideal of polynomials in $\poly{\K}{\Indets}$ whose coefficients add up to zero is a $\symgr{\Indets}$-equivariant ideal,
where $\symgr{\Indets}$ denotes the group of all permutations of permutations $\Indets$.
...
\end{enumerate}
%
The action $\symgr{\Indets}\actson\Indets$ is \kl{nice} but is not \kl{ordered}.
...
In addition to proving finite generation of $\monoid$-equivariant ideals \cite{GHOLAS24} also show that for some specific \kl{nicely ordered} actions $\monoid\actson\Indets$,
one can reduce the $\monoid$-equivariant ideal membership problem to the $\mathcal{N}$-equivariant ideal membership problem for some nicely well-ordered action $\mathcal{N}\actson\Indets$ (we point the reader to \arka{refer} for more details).
However, it is unclear whether their technique can be generalised to the class of \kl{nicely ordered} actions,
at least the authors of \cite{GHOLAS24} do not claim that to be the case.
In particular, it seems difficult to extend the technique of \cite{GHOLAS24} to \kl{nicely ordered} actions $\monoid\actson\Indets$ where $\Indets$ is the structure of a densely coloured linear order/the ordered dense meet tree and $\monoid$ is the group of automorphisms of the structure.
...
by extending $\prec$ to $\mon{\Indets}$ by the co-lex ordering or grevlex ordering
(one must be careful here since some other monomial ordering such as lex are not well-order on $\mon{\Indets}$ when $\Indets$ is infinite).
...
This means that sequence of reductions may not terminate.
%
\arka{maybe it's too early for an example}
\begin{example}
...
\end{example}
%
\arka{fin-gen not possible, so orb-fin gen}


To make reductions terminating we disallow reductions to introduce new variables.
But now for a set $\basis$ of polynomials to be a Gr\"{o}bner basis of an ideal $\idl[I]$,
the leading monomial of $\idl[I]$
%
