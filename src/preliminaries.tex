...
\todo[inline]{This is way too technical. 
  the applications when using infinite number of indeterminates 
  should be in the introduction stating ``growing interest''.
I strongly advise againts putting a proof sketch (it is going to be bad) of another paper (does not serve our narrative)... this is 
  clearly not possible to be understood at this stage. Also, you add again 2/3
  pages before examples and proofs with definitions that will never be used again
  and even worse, closely resemble the actual ones we will be using
  (shortlex / co-lex, reduction game / changing indeterminates, well-founded-ordering preserved / total orderding preserved).
  My suggestions would develop the ``artificial assumptions''
  of the introduction to say: ok, they need a well-founded ordering 
  to be preserved, and they can also deal with equality using a trick.
  While already very useful (state results), this does not give a 
  total answer and leads to tedious proofs.
}
%
\arka{Maybe put it before}
%
...
In \cite{HKL18}, the authors showed that one can even take a submonoid $\calM$ of $\inc{<}$ and prove existence and computability of finite Gr\"{o}bner basis assuming that $\gdivleq[\calM]$ is a well-partial-order.

\todo[inline]{The reference does not explain the result: this is very misleading, 
I really expected to be told how to add the well-founded ordering...}
...
The ideas that were useful in the well-ordered case can not used in directly,
since now even though we can define leading monomials,
we can order them by a well-order preserved by the action of $\group$.
As a consequence, a sequence of reductions which decreases the leading monomial may not be terminating.
To take care of this \cite[GHOLAS24] considers only those reductions that do not introduce new variables.
As described by the proof of \cite[Theorem 12]{GHOLAS24},
the assumption that $\gdivleq$ is a \kl{WQO} ensures that every equivariant ideal $\idl{I}$ contains a finite basis, using which every element of the ideal can be reduced to $0$,
even when reductions are not allowed to introduce new variables.
Thus existence of a finite basis is proven.
Finite subsets such as $B$ of equivariant ideals $\idl[I]$ are called equivariant Gr\"{o}bner bases in this paper.
Strictly speaking, we consider orbit-finite bases and not finite one.
However, a finite basis up to the action of $\group$ is same as an orbit-finite basis.

The Buchberger algorithm can be extended to the equivariant setting (\Cref{alg:weakgb}) and terminates due to the fact that $\gdivleq$ is a \kl{WQO}.
However, it does not give an equivariant Gr\"{o}bner bases.
The intuitive reason behind this is that \kl{$S$-polynomials} only considers cancellation of leading monomials, and not cancellation of variables.
We show how to take care of this in \Cref{sec:equivariant-grobner-basis}

\arka{Maybe put the intuition in a remark after the proof}
...
\arka{Add other orbit-finite research: weighted automata, equations, linear programming}
