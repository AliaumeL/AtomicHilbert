...
\intro{effectively oligomorphic} if for every $n \in \N$ and every \kl{orbit
finite set} $E \subseteq \Indets$, the set $E^n$ is \kl{orbit finite} under the
action of $\group$ on $\Indets^n$.

\arka{Also need to add that we can compute the representatives of orbits}

Furthermore, we ask that, for all $n \in
