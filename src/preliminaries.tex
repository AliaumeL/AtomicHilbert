...
\arka{Maybe we can simply assume $\group$ is a subgroup of bijections of $\Indets$.
Then we can also use the notation $\pi(x)$ which is less confusing than $\pi\cdot x$}
...
\todo[inline]{for arka: I realize this definition is just wrong no? At least
it does not fit with the one given in \cite{BOKLMO21} we do have to introduce the 
