...
The papers \cite{LAUSNO23,GHOLAS24} give necessary and sufficient conditions for an action $\monoid\actson\Indets$ to have the Hilbert's basis property.
...
If $\monoid\actson\Indets$ has the Hilbert's basis property then it is \kl{nice}.
\end{theorem}
%
The ideas used to prove this theorem will be useful in many of our proofs.
Thus, we do a quick recall of the proof.
%
\begin{proof}

<<<<<<< HEAD
\AP A monoid action $\monoid \actson \Indets$ is called \intro{nice} if $(\mon{\Indets}, \gdivleq_{\monoid})$ is \kl{well-quasi-ordered}.
%
\begin{theorem}[{\cite[Theorem 11]{GHOLAS24}}]
If a monoid action has the \kl{Hilbert's basis property} then it is \kl{nice}
\end{theorem}
%
...
%
\begin{theorem}[{\cite[Theorem 12]{GHOLAS24}, \cite[theorem ??]{LAUSNO23}}]
If a monoid action is \kl{nicely ordered} then it has the \kl{Hilbert's basis property}.
\end{theorem}
=======
\end{proof}
%
The action $\monoid\actson\Indets$ is said to \intro{preserve a total order} $\varlt$ on $\Indets$ if 
>>>>>>> d7098fa (conflicting changes)
%
