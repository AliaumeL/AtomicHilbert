...
The meticulous reader can find a proof in the \arka{move the proof to appendix and refer to it}.
%
...
Let $\idl_k$ be the ideal generated by $\bigcup_{i=0}^k \orbit[\monoid]{\monelt_i}$.
Clearly, $\idl_k \subseteq \idl_{k+1}$ for all $k\in\N$.
Pick $n \geq 1$ such that $\idl_{n-1} = \idl_{n}$.
Such an $n$ must exists because otherwise we have a strictly increasing chain of $\monoid$-ideals $\idl_0\subsetneq\idl_1\subsetneq\idl_2\dots$ contradicting the Hilbert's basis property of $\monoid\actson\Indets$.
Since $\idl_{n-1} = \idl_{n}$, $\monelt_{n}\in\idl_{n-1}$.
...
Obviously, every $\mathcal{N}$-equivariant subset is also $\monoid$-equivariant.
...
Every \kl{nice} (injective) monoid action is \kl{nicely orderable}.
\end{conjecture}
%
...
We recall some examples.
%
\begin{example}[Equivariant monomial orderings]
\begin{enumerate}
\item \textbf{Lexicographic ordering}
\item \textbf{Co-lexicographic ordering}
\item \textbf{
\end{enumerate}
\end{example}
%
\arka{remark on why we can not assume well-ordering globally, and that this is a difference with the well-ordered case which makes this case more difficult}
%
