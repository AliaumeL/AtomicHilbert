...
We give in \Cref{ex:idl-equiv} examples and non-examples of $\monoid$-\intro{equivariant ideals}.
%
\begin{example}\label{ex:idl-equiv}
The set $S_0 \subset \poly{\K}{\Indets}$ of all polynomials whose set of coefficients sums to $0$ is an $\symgr{\Indets}$-equivariant ideal.
The set $M_x$ of all polynomials that are multiple of a fixed $x \in \Indets$ is an \kl{ideal} that is not \kl{equivariant}.
\end{example}
%
\begin{proof}
...
%
\begin{example}
\arka{find and shift the example on $\symgr{A}\actson A^2$ here}
\end{example}
