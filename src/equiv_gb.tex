...
%
\begin{remark}
Since we do not restrict an $\monoid$-monomial ordering to be a well-ordering on $\mon{\Indets}$,
such an ordering is not a monomial ordering in the classical sense (\arka{cite cox}).
This relaxation is necessary as for many common examples of monoid actions $\monoid\actson\Indets$ it is not possible to define a monomial ordering on $\mon{\Indets}$ that is $\monoid$-equivariant.
...
\end{example}
%
\begin{lemma}\label{lem:monlt loc}
A total ordering $\monlt$ of $\mon{\Indets}$ is a $\group$-monomial ordering if and only if
\begin{enumerate}
\item $\monlt$ is $\monoid$-equivariant, and
\item $(\mon{S},\monlt)$ is a monomial ordering \arka{cite the definition of monomial ordering} for every finite $S\subseteqfin \Indets$.
\end{enumerate}
\end{lemma}
%
...
Let $f = x^2y^3 - \frac{1}{2}y^2z\in\poly{\Q}{\Indets}$.
...
\arka{reduction $\mapsto$ multivariate division?}
...
The relation $\toeucl{H}$ is an $\monoid$-equivariant relation,
i.e.,
...
\arka{Put a remark that being normalised is not an equivariant property}
...
then there exists a $\monoid$-equivariant function \intro*{$\redfun{H}$} such that for every $f\in\poly{\K}{\Indets}$:
\begin{enumerate}
...
\end{lemma}
%
For every $f\in\poly{\K}{\Indets}$ there exists unique $r$ and such that $r = \redfun{H}^n(f)$ for some $n$ and $\redfun{H}(r) = r$.
...
%\end{proof}
%%
\subsection{Definition of strong Gr\"{o}bner bases}
%
\arka{No need to assume $\monoid$-ideal. Put a remark in the end.}
A subset $\Basis$ of an ideal $\idl[I]$ in $\poly{\K}{\Indets}$ is called an \intro{strong Gr\"{o}bner basis} of $\idl[I]$ if for every $f\in\idl[I]$ there exists $b\in\Basis$ such that
\begin{enumerate}
\item $\dom(b) \subseteq \dom(f)$ and,
\item $\lm[\monlt](b) \divleq \lm[\monlt]{}(f)$.
\end{enumerate}  
%
\begin{example}
The set $\setof{x - 1}{x\in\OrderAtoms}$ is a \kl{strong Gr\"{o}bner basis} of the ideal $\idl[S]_0\subseteq \poly{\Q}{\OrderAtoms}$ containing all polynomials whose coefficients sum up to $0$.

The set $\Basis[C] = \setof{x^2 - 1}{y \in \OrderAtoms}\cup\setof{x^2 - y}{x < y \in \OrderAtoms}$ is subset of $\idl[S]_0$ that generated $\idl[S]_0$ but is not a \kl{strong Gr\"{o}bner basis} of $\idl[S]_0$.
In particular, for $u \in \OrderAtoms$,
the polynomial $u - 1$ is in $\idl[S]_0$ but there does not exists $g\in\Basis[C]$ such that $\dom(g) \subseteq \{u\}$ and,
$\lm[\monlt](g) \divleq u$.
\end{example}
%
\begin{lemma}\label{lem:grob gen idl}
If $\Basis$ is an \kl{strong Gr\"{o}bner basis} of $\idl[I]$,
then $\IdlGen{\Basis} = \idl[I]$.
\end{lemma}
%
The following definition will be used in the proof of the above lemma and will be crucial in the rest of the paper.
%
\begin{definition}\label{def:ch mon}
The \intro{characteristic} monomial of a polynomial $f\in\poly{\K}{\Indets}$ with 
$\dom{f} = \{x_1,\dots,x_n\}$ and $\lm[\monlt](f) = x_1^{k_1} \dots x_n^{k_n}$ is defined as
\[
\cm[\monlt](f) = x_1^{k_1 + 1} \dots x_n^{k_n + 1}
\] 
\end{definition}
%
\begin{example}
For $x \varlt y \varlt z$,
$\cm[\colexlt](x^2y^3z + yz^3) = xy^2z^4$.
\end{example}
%
\begin{proof}[Proof of \Cref{lem:grob gen idl}]
\arka{TODO}
\end{proof}
%
As noted in \Cref{lem:monlt loc},
for every finite subset $S$ of $\Indets$,
the restriction of $\monlt$ to $\mon{S}$ is a monomial ordering.
\arka{maybe worth repeating the definition in the appendix}
Therefore, one can talk about Gr\"{o}bner bases of ideals in $\poly{\K}{S}$ for finite subsets $S$ of $\Indets$.

The next lemma gives a local criterion for a subset of an $\monoid$-ideal to be an $\monoid$-Gr\"{o}bner basis.
%
\begin{lemma}
$\Basis$ is an $\monoid$-Gr\"{o}bner basis of an $\monoid$-ideal $\idl[I]$,
if and only if $\Basis\cap\poly{\K}{S}$ is a Gr\"{o}bner basis of $\idl[I]\cap\poly{\K}{S}$ for every finite subset $S$ of $\Indets$.
\end{lemma}
%
\begin{proof}
\arka{TODO}
...
then every $\monoid$-ideal $\idl[I]\subseteq \poly{\K}{\Indets}$ has an $\monoid$-orbit-finite $\monoid$-Gr\"{o}bner basis.
\end{theorem}
%
\arka{add setwise Gr\"{o}bner bases for nicely orderable actions as well}.
