...
\end{fact}
%
\arka{this is not if and only if in the case when we have a monoid instead of group. Put a remark explaning this with an example.}
%
\begin{remark}
%
...
\item otherwise $f \toeucl{H} \redfun{F}(f)$.
\end{enumerate}
\end{lemma}
%
...
p' = p + r\monelt[q] h
\]
for some $r\in\K$, $\monelt[q]\in\mon[S]$ and $h\in H\cap\poly{\K}{S}$.
%
\begin{claim}
The set $S\cap H$ is finite.
\end{claim}
%
Before proving the claim we show how it implies finiteness of $\rem{H}{f}$.

leveraging König's lemma. This is because 
