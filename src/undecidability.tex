...
groups (\cref{sec:act ex}), the absence of an \kl(of){infinite path} has been
...
Schmitz restated in \cref{conj:wqo-infinite-path}.
...
problem} is undecidable. To that end, we encode the Halting problem of a
...
machine cannot straightforwardly be encoded as monomials due to the
commutativity of the multiplication.


\para{Structures Containing Paths.} \AP Let us assume for the rest of this
...
simplifies the statement and proof of the following
...
we consider the following alphabet $\Gamma \defined \set{ \triangleleft,
  ...
  system}, we encode a run of a reversible Turing machine $M$ on a fixed
  size input tape (\cref{lem:reversible-machine}), and in a second
  \kl{monomial rewrite system}, we create a tape of arbitrary size
  (\cref{lem:tape-creation}). The union of these two \kl{monomial
  rewrite systems} is then used to prove the undecidability of the
...
cannot ensure that we follow one specific \kl(of){finite path}
...
\begin{remark}
  ...
  The undecidability result of \cref{thm:undecidable-paths} generalises
  ...
  such that $(x_i, x_j)$ belongs to one of the orbits $\mathcal{O}_k$
