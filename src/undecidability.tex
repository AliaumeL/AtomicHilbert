...
$\wenc{\varepsilon} \defined 1$, $\wenc{a u}_P = x_0^3 x_1^2 x_2^1 x_3^3
(\mathsf{shift}_{+4} \cdot \wenc{u}_P)$ and $\wenc{b u}_P = x_0^3 x_1^1 x_2^2
...
\begin{lemma}
  ...
  Let $P \defined \seqof{x_i}[0 \leq i < 4n]$ be a finite path in $\Indets$,
  let $u,v,w \in \Sigma^{\leq n}$ be three words, such that $|u| = |v|$,
  and let $\monelt[n] \in \mon{\Indets}$ be a monomial.
  The following are equivalent:
  \begin{enumerate}
    \item \label{item:rewrite-submonomial} 
      There exists $\gelem \in \group$
      such that $\wenc{w}_P = \monelt[m] (\gelem \cdot \wenc{u}_P)$
      and $\monelt[n] = \monelt[m] (\gelem \cdot \wenc{v}_P)$,
