...
\todo[inline]{introduce a function for egb, and weakgb}
We are now ready to write our algorithm to compute an \kl{equivariant Gröbner
basis} by computing the ``congugacy'' $\mathsf{egb} \defined \forgetCol \circ
\mathop{\mathsf{weakgb}} \circ \freeColor$. To prove the correctness of our
...
\begin{lemma}
  ...
  Then $\mathsf{egb}$ is a computable function,
  and the function $\mathsf{weakgb}$ is called 
  on corret inputs.
\end{lemma}
\begin{proof}
