...
basis} of an \kl{equivariant ideal}. To that end, we apply the algorithm
...
disjoint union of two copies of $\Indets$, ordered. It is useful to refer
...
\mathsf{second}$. We also define $\intro*\forgetCol \colon \IndetsCol \to
\Indets$ that maps a coloured variable to its underlying variable.
...
finite subsets $V$ of $\Indets$ of the set $\colorWith{V}(H)$. Note that
...
basis} by computing the ``conjugacy'' 
\begin{equation}
\intro{egb} \defined \forgetCol \circ
\mathop{\kl{weakgb}} \circ \freeColor \quad .
\end{equation}
To prove the correctness of our algorithm,
let us first argue that one can indeed compute the \kl{weak equivariant Gröbner basis}.
...
\begin{proof}
  ...
  we conclude that 
  $\lm(p) = \monelt \lm(h)$, and $\dom(h) \subseteq V = \dom(p)$.
...
\cref{thm:compute-egb},
and in particular we obtain as a corollary that one can decide the \kl{equivariant
...
\csname cor:equivariant-ideals-computations\endcsname*
