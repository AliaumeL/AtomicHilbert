...
\begin{definition}
  ...
  that we write $\domdec(\mathfrak{d})$ is defined as the union
  ...
  $
    \lmdec(\mathfrak{d}) \defined \max(\seqof{\lm(\monelt_i h_i)}[i \in I])
  $.
\end{definition}
...
$\toeucl{H}$. Namely, given two polynomials $p$ and $q$ in $H$, we
