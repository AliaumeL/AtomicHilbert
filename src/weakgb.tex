...
\todo[inline]{This is the translation of the notion of Grobner basis
from polynomials to decompositions}
...
We are now ready to define the saturation algorithm that will compute \kl{weak
equivariant Gröbner bases}, described in
\cref{alg:weakgb}.
Let us remark that \cref{alg:weakgb}
is an actual
algorithm, that can be run on a finite representation of the input set $H$.
Indeed, \todo[inline]{justify it}

\begin{algorithm}
  \caption{Computing \kl{weak equivariant Gröbner bases} using the algorithm \kl{weakgb}}
    \label{alg:weakgb}
