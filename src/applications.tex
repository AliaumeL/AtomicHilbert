...
\todo[inline]{aliaume: add small intro}
...
\todo[inline]{aliaume: simplify the model more}
...
\todo[inline]{aliaume: explain that one could extend the model, but we do not 
do it here.}

\todo[inline]{aliaume: rework the remark}
...
The reachability problem is decidable for reversible Petri net with data $\group\actson\Indets$ if $\group\actson\Indets$ is \kl{nicely orderable}.
\end{corollary}
%
\begin{proof}
...
\]
where $Y\times P$ denotes the product of \kl{WQOs} $Y$ and $P$ (cf. \cite[Section 1.3]{SCSC17}.
\end{proof}
%
\paragraph{Orbit-finite systems of equations}
Let $\group\actson\Indets$ be a group action. Fix a field $\K$ and $n\in\N$.
Let $\lin(\Indets^n)$ denote the vector space generated by $\Indets^n$ over
$\K$. The action of $\group$ extends to the set $\Indets^n$ and then to
$\lin(\Indets^n)$ in the obvious way. An \intro{orbit-finite system of
equations} asks whether a given vector $u\in\lin(\Indets^n)$ is in the vector
space generated by an \kl{orbit-finite} set of vectors $V$ in $\lin(\Indets^n)$
\cite[Section 9]{GHOLAS24}. Theorem 68 in \cite{GHOLAS24} reduces this problem
to the \kl{equivariant ideal membership problem}. As a consequence of this
...
\begin{corollary}\label{cor:lin solv}
  ...
  is decidable for all \kl{nicely orderable} actions $\group\actson\Indets$.
\end{corollary}
%
The above corollary extends \cref{cor:equivariant-ideals-computations}?
%
