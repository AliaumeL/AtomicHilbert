...
\end{example}
%
\arka{write that now we present a structure for which it was not shown}
%
...
%
\cite[Theorem 7.6]{BOJAN16inf} implies that the structures mentioned in  \Cref{ex:eq atoms,ex:dlo,ex:rado,ex:bit vector,ex:dense tree} are \kl{oligomorphic}.
...
\arka{WQO implies oligo (cite prev paper).
To have computability we need to give a way to represent the element of the structures and show point (2) and (3) in the definition of oligomorphicity holds under the representation.
Elements of the structure $\Q$ (cf. example ..) ... can be 
The structure can be though as the set of strings over a finite alphabet and can be represented accordingly.
For trees ...
Showing point (2) and (3) in the definition of effective oligomorphicity holds for this representation is a tedious exercise and hence is left for the reader
}
%
