...
\arka{cite wikipedia and macpherson survey}
...
We leave it to the reader to prove that for every $\monelt[p],\monelt[q]\in\mon[Y]{\A}$ we have $\monelt[p] \gdivleq[\aut{\A}] \monelt[q]$ if and only if the multiset of coefficients of $\monelt{p}$ is smaller than or equal to the multiset of coefficients of $\monelt{1}$ in the multiset ordering (\arka{cite MPRI lecture notes?}).
...
From a monomial $\monelt[p]$ in $\mon[Y]{\D}$ we get a word in $Y^*$ by writing all the exponents in the order prescribed by the indeterminates.
For example, for every $a < b < c \in \D$ and $u,v\in Y$,
$f_{\D}(a^u b^v c^u) = uvu$.
We leave it to the reader to check that for every $\monelt[p],\monelt[q]\in\mon[Y]{\D}$ we have $\monelt[p] \gdivleq[\aut{\A}] \monelt[q]$ if and only if the word corresponding word to $(\monelt[p])$ is smaller than or equal to the word corresponding word to $(\monelt[q])$ in the scattered subword ordering, which is a \kl{WQO} due to Higman's lemma \cite{HIG52}.
\end{example}
%
\begin{example}\label{ex:dlo}
...
which takes as input a pair of nodes of $\T$ and returns their closest common ancestor \arka{cite atombook}.
...
This follows from Kruskal's theorem \arka{add citation} in the following way.
...
\]
where $\gdivleq[\G]\times\gdivleq[\calH]$ is the product ordering \arka{citation?}.
This implies if $(\mon[Q]{\X},\gdivleq{\G}$ and $(\mon[Q]{\Y},\gdivleq{\calH}$ are \kl{WQOs},
so is $(\mon[Q]{\X\uplus\Y}, \gdivleq[\G\times\calH])$.
\end{example}
%
\begin{example}\label{ex:product}
Let $\G * \calH$ denote the free product of the groups $\G$ and $\calH$.
\arka{cite wikipedia}
We extend the group actions $\G\actson\X$ and $\calH\actson\Y$ to a group action $(\G * \calH)\actson(\X\times\Y)$.
Noting that it is enough to define the actions on $\G,\calH\subseteq \G * \calH$,
we define 
\[
\pi((x,y)) =
\begin{cases}
(\pi(x),y) & \text{if }\pi\in\G,\\
(x,\pi(y)) & \text{if }\pi\in\calH.
\end{cases}
\]
$(\mon[Q]{\X\times\Y}, \gdivleq[\G * \calH])$ is not a \kl{WQO} even when $Y$ is a singleton
\arka{finish}
\end{example}
