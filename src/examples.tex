...
\paragraph{Reducts}
%
A group action $\group\actson\Indets$ is said to be a \intro{reduct} of another group action $\calH\actson\Y$ if there exists a bijection $f : \Indets\to\Y$ such that $f^{-1}\circ\pi\circ f \in\group$ for every $\pi\in\calH$.
We write it in symbols as
\[
\group\actson\Indets \reduct{f} \calH\actson\Y \ .
\]
It is called a \intro{effective reduct} if $f$ is \kl{effective}.
%
...
\end{lemma}
%
Call an action $\group\actson\Indets$ \intro{nicely orderable} if it is an \kl{effective reduct} of some action computably oligomorphic action $\calH\actson\Y$ that satisfies the conditions mentioned in \Cref{thm:compute-egb},
i.e.\
$\calH\actson\Y$ preserves an effective linear order and $\gdivleq[\calH]$ is a \kl{WQO}.
As an immediate corollary of \Cref{thm:compute-egb}, \Cref{lem:reducts-equiv-hilbert} we get
%
\begin{corollary}\label{cor:egb}
If $\group\actson\Indets$ is \kl{nicely orderable} then one can compute \kl{equivariant Gröbner bases} of \kl{equivariant ideals}.
\end{corollary}
%
